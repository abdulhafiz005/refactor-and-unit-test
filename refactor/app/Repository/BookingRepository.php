...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs(User $user)
    {
        ...
        foreach ($jobs as $jobitem) {
            // Used ternary operators to filter jobs based on the 'immediate' value.
            $emergencyJobs[] = $jobitem->immediate === 'yes' ? $jobitem : null;
            $noramlJobs[] = $jobitem->immediate !== 'yes' ? $jobitem : null;
        }
        ...
        //array_filter() to remove null entries from the 'emergencyJobs' array
        return ['emergencyJobs' => array_filter($emergencyJobs), 'noramlJobs' => $noramlJobs, 'cuser' => $user, 'usertype' => $usertype];
    }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        $usertype = $cuser->isCustomer() ? 'customer' : ($cuser->isTranslator() ? 'translator' : '');
    ...
    public function store(User $user, array $data)
    {
        ...
        // Validation checks for regular booking
        if ($data['immediate'] == 'no') {
            if (empty($data['due_date']) || empty($data['due_time'])) {
                return ['status' => 'fail', 'message' => "Du måste fylla in alla fält", 'field_name' => ["due_date", "due_time"]];
            }
        ...
        // Set default values for phone and physical types
        ...
        // Set gender and certified based on user selection
        $data['gender'] = $this->getUserSelection($data['job_for'], ['male', 'female']);
        $data['certified'] = $this->getUserSelection($data['job_for'], ['normal', 'certified', 'certified_in_law', 'certified_in_helth']);
    ...
    private function getUserSelection(array $selections, array $options)
    {
        foreach ($options as $option) {
            if (in_array($option, $selections)) {
                return $option;
            }
        }
        return null;
